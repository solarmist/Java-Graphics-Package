...
public class MouseControl implements MouseMotionListener, MouseListener, MouseWheelListener
{
	...
	MouseControl(ObjectPanel panelRef)
	{
		panel = panelRef;
	}
...
END FIX ME RRM */
	...
	{
		if (evt.getButton() == evt.BUTTON1) { //ie left click
			moving = false;
		...
		else if (evt.getButton() == evt.BUTTON3) { //ie right click
			rotating = false;
	 ...
	 * by premultiplying the vector by the inverse transpose of the camera view matrix.
